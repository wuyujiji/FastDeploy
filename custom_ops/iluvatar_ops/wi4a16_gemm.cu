...
std::vector<paddle::Tensor> WI4A16Gemm(
  ...
  int64_t k = x_dims[1];
  int64_t n = ws_dims[1];
  PD_CHECK(n % 2 == 0 && k % group_size == 0);

  PD_CHECK(w_dims[0] == n / 2);
  PD_CHECK(w_dims[1] == k);
...
std::vector<std::vector<int64_t>> WI4A16GemmInferShape(
...
std::vector<paddle::DataType> WI4A16GemmInferDtype(
    const paddle::DataType& input_dtype) {
  return {input_dtype};
}

PD_BUILD_STATIC_OP(wi4a16_gemm)
    .Inputs({"x",
             "weight",
             "weight_scale",
             "weight_zeros",
    ...
    .Attrs({"group_size:int",
            "act_type:std::string"})
    .SetKernelFn(PD_KERNEL(WI4A16Gemm))
    .SetInferShapeFn(PD_INFER_SHAPE(WI4A16GemmInferShape))
    .SetInferDtypeFn(PD_INFER_DTYPE(WI4A16GemmInferDtype));