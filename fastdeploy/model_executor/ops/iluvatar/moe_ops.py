...
try:
    ...
    w4a16_group_gemm = None
    w4a16_group_gemv = None
...
def _pre_process_expert_ffn(
    ...
    if quant_method == "weight_only_int8":
        ...
        tokens_per_expert = restore_tokens_per_expert(tokens_expert_prefix_sum).to("int32")
        if moe_phase == "decode":
            group_gemm_func = w4a16_group_gemv
        else:
            group_gemm_func = w4a16_group_gemm
...
def iluvatar_moe_expert_ffn(
    ...
    assert quant_method in ("weight_only_int4", "weight_only_int8")
    if quant_method == "weight_only_int4":
        assert up_gate_proj_zeros is not None
        assert down_proj_zeros is not None
    print(f"iluvatar_moe_expert_ffn: group_size={group_size}")
    group_gemm_func, tokens_per_expert = _pre_process_expert_ffn(
        moe_phase,
        quant_method,
