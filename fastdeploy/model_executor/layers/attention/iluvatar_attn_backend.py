...
class IluvatarAttnBackend(AttentionBackend):
    ...
    def init_attention_metadata(self, forward_meta: ForwardMeta):
        ...
        if prefill_batch_ids.dim() == 0:
            with nvtx.range("prefill_batch_ids_unsqueeze"):
                prefill_batch_ids = prefill_batch_ids.unsqueeze(0)
        if decode_batch_ids.dim() == 0:
            with nvtx.range("decode_batch_ids_unsqueeze"):
        ...
        # both prefill and decode
            ...
            with nvtx.range("prefill_and_decode"):
                self.mixed = True
                with nvtx.range("split_cos_sin"):
                ...
                with nvtx.range("generate_id_group"):
    ...
    def forward_mixed(
        ...
        if self.decode_len == 0:
            with nvtx.range("prefill_fused_paged_attention"):
            ...
            with nvtx.range("decode_fused_paged_attention"):
            ...
            with nvtx.range("mixed_fused_paged_attention"):
