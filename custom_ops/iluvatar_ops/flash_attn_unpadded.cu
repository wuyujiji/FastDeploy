...
std::vector<paddle::Tensor> FlashAttnUnpadded(
  ...
  auto out =
      paddle::empty({num_tokens, num_heads, head_dim}, dtype, q.place());

  switch (dtype) {
    case paddle::DataType::BFLOAT16:
      FlashAttnUnpaddedKernel<paddle::DataType::BFLOAT16>(
          q,
          k,
          v,
          cu_seqlens_q,
    ...
    case paddle::DataType::FLOAT16:
      FlashAttnUnpaddedKernel<paddle::DataType::FLOAT16>(
          q,
          k,
          v,
          cu_seqlens_q,
...
PD_BUILD_STATIC_OP(flash_attn_unpadded)
    .Inputs({"q",
             "k",
             "v",
             "cu_seqlens_q",
