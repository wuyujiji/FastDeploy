...
def limit_thinking_content_length(
    ...
    if limit_strategy == "</think>":
        # for ernie-45-vl
        with nvtx.range("limit_thinking_content_length_v1"):
            limit_thinking_content_length_v1(
                sampled_token_ids,
                max_think_lens,
                step_idx,
        ...
        with nvtx.range("limit_thinking_content_length_v2"):
            limit_thinking_content_length_v2(
                sampled_token_ids,
                max_think_lens,
                step_idx,
...
def pre_process(
    ...
    Return:
        ...
        cu_seqlens_k:
    """
    with nvtx.range("get_token_num_cpu"):
...
def post_process_normal(
    ...
    if think_end_id > 0:
        with nvtx.range("limit_thinking_content_length"):
    ...
    # 1. Set stop value
    with nvtx.range("where_and_assign"):
        paddle.assign(
            paddle.where(
    ...
    if not skip_save_output:
        if envs.FD_USE_GET_SAVE_OUTPUT_V1:
            if save_each_rank or model_output.mp_rank == 0:
                with nvtx.range("_build_stream_transfer_data"):
...
def post_process(
    ...
    if isinstance(sampler_or_pooler_output, PoolerOutput):
        ...
        if speculative_decoding:
            ...
            with nvtx.range("post_process_normal"):
                post_process_normal(
                    sampler_or_pooler_output,
                    model_output,
                    share_inputs,
